package com.darkmentor.ui.devicedetails
...
import com.darkmentor.bottomRight
...
import com.darkmentor.domain.model.toGeoPoint
import com.darkmentor.dpToPx
import com.darkmentor.extract16BitUuid
import com.darkmentor.frameRate
import com.darkmentor.pxToDp
import com.darkmentor.ui.AsyncBatchProcessor
import com.darkmentor.ui.map.MapView
import com.darkmentor.utils.ScreenSizeLocal
import com.darkmentor.utils.graphic.DevicePairedIcon
import com.darkmentor.utils.graphic.DeviceTypeIcon
import com.darkmentor.utils.graphic.ExtendedAddressView
import com.darkmentor.utils.graphic.ListItem
import com.darkmentor.utils.graphic.RadarIcon
import com.darkmentor.utils.graphic.RoundedBox
import com.darkmentor.utils.graphic.SignalData
...
import com.darkmentor.utils.graphic.infoDialog
...
object DeviceDetailsScreen {
    ...
    private fun ServiceDetails(service: DeviceDetailsViewModel.ServiceData, viewModel: DeviceDetailsViewModel) {
        ...
        Column(
            ...
            // Both GATT card backgrounds (GATT_SERVICE_BG light blue, GATT_CHARACTERISTIC_BG
            // light green) are fixed light colours regardless of theme. The default text/icon
            // colour is LocalContentColor (theme onSurface), which is white in dark mode and
            // unreadable here. Force black content for the whole card subtree — the nested
            // characteristic cards are equally light, so one provider at the service level covers
            // every Text title, value, hex line, and the expand chevron.
        ...
        ExpandableLine(
            ...
            Column {
                ...
                if (isReadable || hasCachedValue) {
                    if (hasCachedValue) {
                        if (value != null) Text(value, color = Color.Black)
                        Text(valueHex!!, color = Color.Black)
    ...
    private fun UuidTitle(uuid: String, name: String?, clues: DeviceDetailsViewModel.CluesInfo?) {
        ...
        Column {
            // Explicit black: these sit on the fixed light GATT card backgrounds. The default
            // text style carries the theme's onSurface colour (white in dark mode), which wins
            // over LocalContentColor in Material3 Text — so the colour param is the only reliable
            // override here.
            Text(text = "$uuid - $resolvedName", fontWeight = FontWeight.Bold, color = Color.Black)
            clues?.company?.let { Text(text = it, fontWeight = FontWeight.Light, color = Color.Black) }
        }
    }
    ...
    private fun CluesPurpose(purpose: String) {
        Column(modifier = Modifier.padding(start = 8.dp, top = 4.dp, bottom = 4.dp)) {
            Text(text = stringResource(R.string.clues_purpose_label), fontWeight = FontWeight.SemiBold, color = Color.Black)
            Text(text = purpose, color = Color.Black)
        }
    }
